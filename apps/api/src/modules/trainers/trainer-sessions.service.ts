...
import { defaultSlotTimes, slotStartEndFor, weekdayShort } from "./trainer-slot-utils";
...
export class TrainerSessionsService {
  ...
  private allowedStartsForTrainer(trainer: {
    availability: unknown;
  }, dateIso: string): string[] {
    const d = new Date(`${dateIso}T12:00:00Z`);
    const day = weekdayShort(d);
    const raw = trainer.availability as { day?: string; slots?: string[] }[] | null;
    if (!Array.isArray(raw) || raw.length === 0) {
      return defaultSlotTimes();
    }
    const entry = raw.find((x) => (x.day ?? "").toLowerCase() === day.toLowerCase());
  ...
  async createBooking(
    ...
    return mutexFor(slotId).runExclusive(async () =>
      this.createBookingExclusive(dto, authenticatedUserId, trainer),
    );
  }
  ...
  async cancelBooking(bookingId: string, userId: string): Promise<TrainerBookingDto> {
    const row = await this.prisma.trainerBooking.findUnique({ where: { id: bookingId } });
    if (!row) {

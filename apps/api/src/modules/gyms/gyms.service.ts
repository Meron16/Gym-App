...
import { bboxForLocationQuery, fetchFitnessPlacesFromOSM } from "./osm-overpass";
...
export class GymsService {
  ...
  private capacityPercent(base: number): `${number}%` {
    return `${capacityPercentForNow(base)}%` as `${number}%`;
  }
  ...
  async search(query: GymSearchQueryDto): Promise<GymSummaryDto[]> {
    ...
    const filtered = rows
      .filter((row) => {
        ...
        const matchesQ = !q || row.name.toLowerCase().includes(q) || row.tag.toLowerCase().includes(q);
        const matchesLoc = !location || row.location.toLowerCase().includes(location);
        const matchesFac =
          !facility || amenities.some((a) => String(a).toLowerCase().includes(facility));
      ...
      .map((row) => ({
        row,
        distanceKm: hasGeo ? haversineKm(lat, lng, row.lat, row.lng) : Number.POSITIVE_INFINITY,
      }));
  ...
  async detail(id: string): Promise<GymDetailDto> {
    ...
    return {
      ...
      operatingHours: (Array.isArray(row.operatingHours) ? row.operatingHours : []) as {
        day: string;
        open: string;
        close: string;
      }[],
   ...
   */
  async syncOsmVenues(locationQuery?: string): Promise<{ locationQuery: string; upserted: number }> {
    ...
    for (const el of elements) {
      ...
      const capacityBase = 35 + (hashString(`${el.type}_${el.id}_${name}`) % 60);
